...
void update_mini_batch(network_t *net, set_loader_t *loader,
  ...
  for (int l = 0; l < net->num_layers-1; l++) {
    ...
    gsl_matrix_scale(weight_grads->data[l], scaler);
    gsl_matrix_sub(net->weights[l], weight_grads->data[l]);

    gsl_matrix_scale(bias_grads->data[l], scaler);
    gsl_matrix_sub(net->biases[l], bias_grads->data[l]);
  }
...
int evaluate(network_t *net, set_loader_t *test_loader) {
  ...
  image_t *img;
  int sum = 0;
  // activations = init_activations(net);
  // outputs = init_outputs(net);
  ...
  // gsl_matrix_list_free_matrices(outputs);
  return sum;
}
