...
void stochastic_gradient_descent(network_t *net, set_loader_t *train_loader,
  ...
  int num_images = loader->total;
  int mini_batches = total/mini_batch_size;
  for (int e = 0; e < epochs; e++) {
    shuffle(loader);
    for (int m = 0; m < mini_batches; m++) {
      printf("Batch %d\r", m+1);
...
void update_mini_batch(network_t *net, set_loader_t *loader,
  ...
  delta_weight_grads = gsl_matrix_list_malloc(net->num_layers-1);
  delta_bias_grads = gsl_matrix_list_malloc(net->num_layers-1);
  ...
  for (int l = 1; l < net->num_layers; l++) {
    weight_grads->data[l-1] = gsl_matrix_calloc(net->layers[l], net->layers[l-1]);
    bias_grads->data[l-1] = gsl_matrix_calloc(net->layers[l], 1);
  }
  ...
  for (int i = 0; i < mini_batch_size; i++) {
    ...
    for (int l = 0; l < net->num_layers-1; l++) {
      ...
      gsl_matrix_sub(net->biases[l], delta_weight_biases->data[l]);
    }
  }
...
double evaluate(network_t *net, set_loader_t *test_loader) {
  ...
  image_t *img;
  double sum = 0;
  for (size_t m = 0; m < test_loader->total) {
    img = get_next_image(test_loader);
    input = image_to_matrix(img, test_loader->height, test_loader->width);
    activations = gsl_matrix_list_malloc(num_layers);
    outputs = gsl_matrix_list_malloc(num_layers-1);
    final_activation = gsl_matrix *feedforward(net, input, activations, outputs);
