...
void backprop(network_t *net, gsl_matrix *input, gsl_matrix *target,
  ...
  // for (int i = 0; i < weight_grads->length; i++) {
  //   printf("%s\n", );
  //   assert(same_shape(weight_grads->data[i], net->weights[i]));
  //   assert(same_shape(bias_grads->data[i], net->biases[i]));
  // }
  ...
  for (int l = 2; l < net->num_layers; l++) {
    ...
    // net->weights[(wgrad_size-l+1)] = gsl_matrix_alloc(
    //                                       (net->weights[(wgrad_size-l+1)])->size2,
    //                                       delta->size2);
